$(document).ready(function() {
    ...
    async function getRandomPokemon(count = 12) {
        ...
        return (await Promise.all(pokemonPromises)).filter(pokemon => pokemon !== null);
    }

    async function getRandomPokemonByType(type) {
        try {
            const typeData = await fetchData(`${API_BASE_URL}type/${type}`);
            const randomPokemon = typeData.pokemon[Math.floor(Math.random() * typeData.pokemon.length)].pokemon;
    ...
    function updatePagination(totalPokemon) {
        ...
        if (totalPages > 1) {
            pagination.append(`<button class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 transition duration-300 prev-page ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''}">Previous</button>`);
            
            for (let i = 1; i <= Math.min(5, totalPages); i++) {
                pagination.append(`<button class="px-4 py-2 ${currentPage === i ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700'} rounded-md hover:bg-gray-300 transition duration-300 page-number" data-page="${i}">${i}</button>`);
            }

            pagination.append(`<button class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 transition duration-300 next-page ${currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : ''}">Next</button>`);
        }
    }

    // Event listener for pagination
    $('#pagination').on('click', '.page-number, .prev-page, .next-page', function(e) {
        e.preventDefault();
        if ($(this).hasClass('prev-page') && currentPage > 1) {
            currentPage--;
        } else if ($(this).hasClass('next-page') && currentPage < Math.ceil(allPokemonData.length / pokemonPerPage)) {
            currentPage++;
    ...
    async function searchPokemon(query) {
        showLoading();
        try {
            const pokemon = await fetchData(`${API_BASE_URL}pokemon/${query.toLowerCase()}`);
            const pokemonList = $('#pokemon-list');
            pokemonList.empty();
            pokemonList.append(createPokemonCard(pokemon));
            $('#pagination').hide();
                    ...
                    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
                        ...
                        <div class="mt-4">
                            ...
                            ${pokemon.stats.map(stat => `
                                <div class="mt-2">
                                    <span class="font-semibold">${stat.stat.name}:</span>
    ...
    function createEvolutionChain(chain) {
        let evolutionHtml = `<div class="text-center"><img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${chain.species.url.split('/')[6]}.png" alt="${chain.species.name}" class="w-20 h-20 mx-auto"><p class="capitalize">${chain.species.name}</p></div>`;
        
        if (chain.evolves_to.length > 0) {
            evolutionHtml += `<div class="text-4xl">&rarr;</div>` + createEvolutionChain(chain.evolves_to[0]);
        }
    ...
    async function getRelatedPokemon(pokemon) {
        ...
        return relatedPokemon.pokemon
            .slice(0, 5)
            .map(p => `<div class="text-center"><img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${p.pokemon.url.split('/')[6]}.png" alt="${p.pokemon.name}" class="w-16 h-16"><p class="text-xs capitalize">${p.pokemon.name}</p></div>`)
            .join('');
    }
    ...
    function showLoading() {
        $('body').append('<div id="loading" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center"><div class="animate-spin rounded-full h-32 w-32 border-t-2 border-b-2 border-blue-500"></div></div>');
    }
    ...
    async function initializeTypeFilter() {
        try {
            ...
            const filterDropdown = $('<div id="filter-dropdown" class="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 hidden z-10"></div>');
            
            filterDropdown.append(`<a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 filter-option" data-type="">Clear Filter</a>`);
            
            types.results.forEach(type => {
                filterDropdown.append(`<a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 filter-option" data-type="${type.name}">${type.name}</a>`);
            });
            ...
            filterDropdown.on('click', '.filter-option', function(e) {
                ...
                filterButton.text(currentFilter ? `Filter: ${currentFilter}` : 'Filter');
                filterDropdown.addClass('hidden');
                currentPage = 1;
                loadPokemon(currentPage, currentFilter);
            });
